...
local inline size_t map_size() {
    return (size_t)(1 << (g.max+1)) * (g.total_symbols+1);
}

// Index function for num[] and done[].
// 根据syms、left、len生成一个不碰撞的index作为唯一索引。
// syms: 当前剩余需要编码的符号数量
// left: 当前huffman树的本层，还剩下多少个节点点位可供编码
// len: 当前正在遍历的huffman树层数。
local inline size_t map(int syms, int left, int len) {
    // g.max表示huffman树最大可能的层深度
    return ((size_t)(1<<len) + left) * (g.total_symbols+1) + syms;
...
local big_t count(int syms, int left, int len) {
    ...
    // 符号数等于当前层数，根据完全树的定义，就应该直接往后全部编码到本层即可。只有一种编码方案。
    assert (len <= g.max && "len must not exceed g.max");
    if (len == g.max) {
        assert(syms <= left && "syms must not exceed the left positions.");
        return 1;
    }
    if (syms == 0) {
        return 1;
...
//        return 1;
    ...
    // It's possible to have syms <= left for an incomplete tree.
    assert(left > 0 && len < g.max);
//    assert(syms > left && left > 0 && len < g.max);
    ...
    // 当剩余符号syms不太多时，例如小于本层开放节点数两倍，left*2，时，就需要限定least，
    // 否则生成的不是完全树，不属于本函数的统计范畴。
    // 若小于least，则剩下的符号，即便全部放在len+1层，仍旧不能占据完全的树，再往下就更不可能了。
    int least = 0;
...
//        least = 0;
    ...
    // 最多能使用多少个符号呢，就是本层使用了most个符号之后，剩余所有的符号都放置到max层，这样才能
    // "勉强"刚刚好放下所有的符号，此时，本层分配的节点数就是most。
    // If use>most, then we can not put all the symbols into this tree. No matter it's complete or not!
    int most = (((code_t)left << (g.max - len)) - syms) /
               (((code_t)1 << (g.max - len)) - 1);
    if (most > syms) {
        // If syms > left, then we must have most <= syms and most <= left.
        assert (syms <= left);
        most = syms;  // Thus, implicitly, we have most <= left.
    }
    assert(most <= syms && most <= left);
    assert(most >= 0);

    // count all possible codes from this juncture and add them up
    big_t sum = 0;
    // 利用分治算法，得出全部可能编码方式的数量。相同的层级直方图视为一次，即视为等价类。
...
local int been_here(int syms, int left, int len, int mem, int rem) {
    ...
    rem >>= 1;
    // 这里的offset是怎么保证，任意两组(mem, rem)不碰撞的？？
...
// mem表示目前已经开辟的各个子table外加根table的全部累计表项数目
// rem表示当前一个子huffman table剩余可用的表项数目
local void examine(int syms, int left, int len, int mem, int rem,
                   int init_syms, int init_left, int init_len) {
    assert (len <= g.max && "len must not exceed g.max");
    // see if we have a complete code
    if (len == g.max || syms == 0) {  //(syms == left) {
        if (len == g.max) {
            assert(syms <= left && "syms must not exceed the left positions.");
        }
...
//        g.code[len] = left;
        ...
        if (syms > 0) {  // For the case of len==g.max
            while (rem < syms) {
                syms -= rem;
                rem = 1 << (len - g.root);
                mem += rem;
...
//            }
            ...
            string_printf(&g.out, "<syms=%u, left=%u, len=%u>:",
                          init_syms, init_left, init_len);
...
//        least = 0;
    ...
    // 现在本层，把至少需要完整消耗掉的sub-table用掉。例如，第二层，sub-table为2^2 = 4，那么可能需要很多个sub-table才能消耗掉least。
    int use = least;
    while (rem < use) {
        use -= rem;
        rem = 1 << (len - g.root);
    ...
    for (use = least; use <= most; use++) {
        g.code[len] = use;
        // 根据当前的假设，继续往下一层继续分配。总之，已知，当前层级的use是基本合理的。
        // 若要往下继续走：
        //   1. 当前的sub-table如果还没用完，那么，sub-table实际上应该多用一倍，即应当加上2^(len-root)，即便这个sub-table是在很多层以前初始化的，这样操作也是对的。
        //      此外，之前用过的空间都要乘以2，相应的，之前剩余的rem也需要乘以2.
        //      归纳如下，当前sub-table已经用的空间x2，剩余未使用的空间x2。而总的memory使用量+2^(len-root)。
        //   2. 当前sub-table已经用完，那么mem和rem都不需要变化。
        assert(syms >= use);
        if (syms > use) {
            ...
            // In this case, we won't add mem by 2^(len-root)
            examine(syms - use, (left - use) << 1, len + 1,
                    mem, rem << 1,
                    init_syms, init_left, init_len);
        }
...
//            for (int left = 2; left < n; left += 2) {
                ...
                // 这里被我comment掉了，两个原因：1，原理不清楚。2，最大内存占用的情况，不大可能出现在这里。考虑一般情况哈！特殊情况就算了。
...
int main(int argc, char **argv) {
    ...
    int syms = 40;
    g.total_symbols = syms;
    g.root = 8;
    g.max = 15;
    if (argc > 1) {
        g.total_symbols = syms = atoi(argv[1]);
        if (argc > 2) {
            g.root = atoi(argv[2]);
            if (argc > 3)
                g.max = atoi(argv[3]);
...
//        g.size *= n;
    ...
    // 计算2~n个符号，的编码方式一共有多少种，并得出全部的总和。只是，这有什么意义？？
    big_t sum = 0;
